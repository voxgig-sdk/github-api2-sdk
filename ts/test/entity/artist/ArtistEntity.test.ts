...
import { GithubApi2SDK, BaseFeature, stdutil } from '../../..'
...
describe('ArtistEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set GITHUB_API__TEST_ARTIST_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  let client = GithubApi2SDK.test(options, extra)
  ...
  const idmapEnvVal = process.env['GITHUB_API__TEST_ARTIST_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'GITHUB_API__TEST_ARTIST_ENTID': idmap,
    'GITHUB_API__TEST_LIVE': 'FALSE',
    'GITHUB_API__TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['GITHUB_API__TEST_ARTIST_ENTID']
