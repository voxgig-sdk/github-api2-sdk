...
func TestArtistEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GITHUBAPI__TEST_ARTIST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func artistBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("GITHUBAPI__TEST_ARTIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GITHUBAPI__TEST_ARTIST_ENTID"])
	...
	if env["GITHUBAPI__TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewGithubApi2SDK(core.ToMapAny(mergedOpts))
	}
