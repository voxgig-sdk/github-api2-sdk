...
func main() {
    ...
    // List artist records — the value is the array of records itself.
    artists, err := client.Artist(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range artists.([]any) {
        fmt.Println(item)
    }
}
```
...
artist, err := client.Artist(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(artist) // the returned mock data
```
...
slice):
    ...
    artist, err := client.Artist(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // artist is the returned record
