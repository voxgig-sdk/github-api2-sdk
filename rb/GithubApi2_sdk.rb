...
require_relative 'GithubApi2_types'
...
class GithubApi2SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GithubApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GithubApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GithubApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    GithubApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GithubApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GithubApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GithubApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => GithubApi2Error.new(
        "#{op}_allow",
        "GithubApi2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GithubApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GithubApi2Error.new(
        "graphql_error", "GithubApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
