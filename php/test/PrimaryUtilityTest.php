...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        GithubApi2SDK $client,
        GithubApi2Utility $utility
    ): GithubApi2Context {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new GithubApi2Context($ctxmap, null);
    ...
    private static function fixctx(GithubApi2Context $ctx, GithubApi2SDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?GithubApi2Error
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GithubApi2Error($code, $msg);
    }
    ...
    {
        $client = GithubApi2SDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new GithubApi2BaseFeature();
    ...
    {
        $hook_client = GithubApi2SDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends GithubApi2BaseFeature {
            public $hook_fn = null;
            public function TestHook(GithubApi2Context $ctx): void
            {
    ...
    {
        $init_client = GithubApi2SDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends GithubApi2BaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new GithubApi2SDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new GithubApi2Spec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = GithubApi2SDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = GithubApi2SDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof GithubApi2Error) ? $result->err->msg : (string)$result->err;
