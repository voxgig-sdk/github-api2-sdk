...
class ImageEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GITHUBAPI__TEST_IMAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function image_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("GITHUBAPI__TEST_IMAGE_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "GITHUBAPI__TEST_IMAGE_ENTID" => $idmap,
        "GITHUBAPI__TEST_LIVE" => "FALSE",
        "GITHUBAPI__TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["GITHUBAPI__TEST_IMAGE_ENTID"]);
