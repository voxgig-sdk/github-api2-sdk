...
class GithubApi2SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = GithubApi2Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = GithubApi2Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ($utility->feature_add)($this->_rootctx, GithubApi2Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return GithubApi2Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = GithubApi2Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = GithubApi2Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = GithubApi2Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new GithubApi2Error($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = GithubApi2Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new GithubApi2Error("graphql_error",
                "GithubApi2SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
