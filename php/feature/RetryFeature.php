...
class GithubApi2RetryFeature extends GithubApi2BaseFeature
{
    ...
    public function init(GithubApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (GithubApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(GithubApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(GithubApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
