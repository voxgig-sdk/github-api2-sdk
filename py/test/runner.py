...
class GithubApi2TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GithubApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GithubApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GithubApi2TestRunner.getenv("GITHUB_API2_TEST_LIVE")
        override = GithubApi2TestRunner.getenv("GITHUB_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GithubApi2TestRunner.getenv(key)
        ...
        explain = GithubApi2TestRunner.getenv("GITHUB_API2_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GithubApi2TestRunner._test_control is not None:
            return GithubApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GithubApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GithubApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GithubApi2TestRunner.load_test_control()
...
def load_env_local():
    GithubApi2TestRunner.load_env_local()


def env_override(m):
    return GithubApi2TestRunner.env_override(m)


def entity_data(v):
    return GithubApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GithubApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GithubApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GithubApi2TestRunner.load_test_control()


def live_delay_ms():
    return GithubApi2TestRunner.live_delay_ms()
